...
impl BaseState {
        ...
        match self.focus {
                ...
                if let Some(adv_idx) = self.selected_adventurer {
                    if adv_idx < roster.adventurers.len() {
                        ...
                        if is_key_pressed(KeyCode::H) {
                            let has_infirmary = kingdom
                                .buildings
                                .iter()
                                .any(|b| b.id == "infirmary" && b.built);
                        ...
                        if is_key_pressed(KeyCode::T) {
                            let has_chapel = kingdom
                                .buildings
                                .iter()
                                .any(|b| b.id == "chapel" && b.built);
            ...
            FocusArea::PartyFormation => {
                ...
                for i in 0..available_count {
                    ...
                    let card_y = roster_y + 35.0 + (i as f32 * 70.0) - 15.0;
                    let card_clicked =
                        crate::ui::was_clicked(roster_x, card_y, card_width, card_height);
    ...
    pub fn draw(
        ...
        draw_text(
            "Manage your kingdom. [TAB] Switch Focus",
            20.0,
            70.0,
            18.0,
            GRAY,
        ...
        draw_text(
            &format!("Supplies: {}", stats.supplies),
            20.0,
            y_start + 25.0,
            20.0,
            YELLOW,
        ...
        draw_text(
            &format!("Security: {}", stats.security),
            20.0,
            y_start + 50.0,
            20.0,
            SKYBLUE,
        ...
        draw_text(
            &format!("Knowledge: {}", stats.knowledge),
            20.0,
            y_start + 100.0,
            20.0,
            PURPLE,
        ...
        let roster_color =
            if self.focus == FocusArea::Roster || self.focus == FocusArea::PartyFormation {
                YELLOW
            } else {
                WHITE
            };
        ...
        for (i, adv) in roster.adventurers.iter().enumerate() {
            ...
            let card_y = y - 15.0;
            let is_selected =
                self.selected_adventurer == Some(i) && self.focus == FocusArea::Roster;
            let is_hovered = crate::ui::is_mouse_over(roster_x, card_y, card_width, card_height);
            let is_in_party =
                self.focus == FocusArea::PartyFormation && self.forming_party.contains(&adv.id);
            let is_leader = self.focus == FocusArea::PartyFormation
                && self.forming_party.leader_id() == Some(&adv.id);
            ...
            let party_mark = if is_in_party && !is_leader {
                "✓ "
            } else {
                ""
            };
            draw_text(
                &format!("[{}] {}{}{}", i + 1, leader_mark, party_mark, adv.name),
                roster_x + 10.0,
                y + 5.0,
                20.0,
                name_color,
        ...
        let build_x = 700.0;
        let build_y = 120.0;
        let build_color = if self.focus == FocusArea::Buildings {
            YELLOW
        } else {
            WHITE
        };
        ...
        for (i, b) in kingdom.buildings.iter().enumerate() {
            ...
            let is_selected =
                self.selected_building == Some(i) && self.focus == FocusArea::Buildings;
            ...
            if b.built {
                ...
                draw_text(
                    &format!("Cost: {}g, {}s", b.cost_gold, b.cost_supplies),
                    build_x + 10.0,
                    y + 25.0,
                    16.0,
                    RED,
        ...
        draw_text(
            "[F5] Save  [F9] Load  [TAB] Switch Focus",
            20.0,
            screen_height() - 20.0,
            16.0,
            GRAY,
